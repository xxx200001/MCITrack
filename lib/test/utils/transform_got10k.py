...
def transform_got10k(tracker_name, cfg_name):
    ...
    for item in items:
        ...
        if "time" not in item:
            ...
            bbox_arr = np.loadtxt(src_path, dtype=np.int64, delimiter='\t',encoding='utf-8')
    ...
    try:
        ...
        print("Error in creating zip archive for source directory:", e)

    try:
        shutil.make_archive(dest_dir, "zip", dest_dir)
    except Exception as e:
        print("Error in creating zip archive for destination directory:", e)
