...
import argparse
import random
os.environ['CUDA_VISIBLE_DEVICES'] = '4,5,6,7'  # 只用这 4 张卡, 绝不碰 0-3
...
def parse_args():
    ...
    # for train
    parser.add_argument('--script', type=str, default='mcitrack', help='training script name')
    parser.add_argument('--config', type=str, default='mcitrack_b224_fusion', help='yaml configure file name')
    ...
    parser.add_argument('--nproc_per_node', default=4, type=int,
                        help="number of GPUs per node")  # specify when mode is multipl
...
def main():
    args = parse_args()
    if args.mode == "single":
        train_cmd = "/home/zhanggt/anaconda3/envs/mcitrack/bin/python lib/train/run_training.py --script %s --config %s --save_dir %s --use_lmdb %d " \
        ...
        train_cmd = "OMP_NUM_THREADS=2 /home/zhanggt/anaconda3/envs/mcitrack/bin/python -m torch.distributed.run --nproc_per_node %d --master_port %d lib/train/run_training.py " \
        ...
        train_cmd = "/home/zhanggt/anaconda3/envs/mcitrack/bin/python -m torch.distributed.run --nproc_per_node %d --master_addr %s --master_port %d --nnodes %d --node_rank %d lib/train/run_training.py " \
                    "--script %s --config %s --save_dir %s --use_lmdb %d --script_prv %s --config_prv %s --use_wandb %d " \
                    "--distill %d --script_teacher %s --config_teacher %s" \
